...
(
	...
	INVITE_PERSON   VARCHAR(30)       NOT NULL DEFAULT ''       COMMENT '邀请人',
...
(
	ID INT AUTO_INCREMENT PRIMARY KEY,
	PHONE_NUMBER        VARCHAR(50) NOT NULL                COMMENT '号码',
	CAPTCHA_TYPE        VARCHAR(20) NOT NULL                COMMENT '验证码类型',
	CAPTCHA_CODE        VARCHAR(20) NOT NULL                COMMENT '验证码',
	DISABLED            INT(5)      NOT NULL DEFAULT 0      COMMENT '是否有效 0-失效 其他-有效',
	EXPIRES_TIME        DATETIME    NOT NULL                COMMENT '失效时间',
	CONSTRAINT UQ_SMS_CAPTCHA UNIQUE (PHONE_NUMBER, CAPTCHA_TYPE, EXPIRES_TIME)
)
COMMENT '短信验证码信息表';

CREATE INDEX IDX_SMS_CAPTCHA_PHONE_NUMBER ON TB_SMS_CAPTCHA (PHONE_NUMBER);
CREATE INDEX IDX_SMS_CAPTCHA_CAPTCHA_TYPE ON TB_SMS_CAPTCHA (CAPTCHA_TYPE);
CREATE INDEX IDX_SMS_CAPTCHA_EXPIRES_TIME ON TB_SMS_CAPTCHA (EXPIRES_TIME);
```

```mysql
DROP TABLE IF EXISTS TB_SMS_ACCESS;
CREATE TABLE TB_SMS_ACCESS
(
	ID INT AUTO_INCREMENT PRIMARY KEY,
	PHONE_NUMBER        VARCHAR(50) NOT NULL                COMMENT '号码',
	SMS_REQUEST         VARCHAR(20) NOT NULL                COMMENT '验证码类型',
	SMS_RESPONSE        VARCHAR(20) NOT NULL                COMMENT '验证码',
	SEND_TIME           DATETIME    NOT NULL                COMMENT '失效时间'
)
COMMENT '短信验证码请求信息表';

CREATE INDEX IDX_SMS_CAPTCHA_PHONE_NUMBER ON TB_SMS_ACCESS (PHONE_NUMBER);
CREATE INDEX IDX_SMS_CAPTCHA_EXPIRES_TIME ON TB_SMS_ACCESS (SEND_TIME);
```
